...
class NetworkNodeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        node_type = data.get(
            "node_type", self.instance.node_type if self.instance else None
        )

        if node_type == NetworkNode.NodeType.FACTORY and supplier:
            raise serializers.ValidationError(
                {
                    "supplier": "Завод всегда находится на уровне 0 и не может иметь поставщика."
                }
            )
        ...
        if self.instance:
            if supplier and supplier.id == self.instance.id:
                raise serializers.ValidationError(
                    {"supplier": "Звено не может быть своим поставщиком."}
                )
